...
def get_plc_data(plc, plc_ip, plc_rack, plc_slot, dbs, starts, lengths, executor):
    ...
    try:
        fw_data_redis_dir['module'] = config[plc_ip_match][plc_ip]
        partition = fw_data_redis_dir['module'][-1]
        data_list = json.dumps(fw_data_redis_dir, ensure_ascii=False)
        if int(fw_data_redis_dir['module'], 10) > 20:
            redis_data.set("fccs", data_list, ex=60)
            plc_data_producer.send('plc_data_kafka', key=plc_ip.encode('utf-8'), value=data_list.encode('utf-8'))
            # print(f"{data_list}--{datetime.now()}")
        else:
            plc_data_producer.send('plc_data_kafka', key=plc_ip.encode('utf-8'), value=data_list.encode('utf-8'))
            redis_data.set(f"飞轮舱{config[plc_ip_match][plc_ip][-1]}", data_list, ex=60)
